...
const features = [
  ...
  { icon: FolderPlus, label: "Multi-format Import", desc: "Import mods from JSON files, ZIP archives, or folders" },
...
const changelog = [
  {
    version: "1.1.0",
    date: "April 2026",
    title: "Three-Way Merge Engine, Contextual Pattern Scan & Mod Bundle Support",
    changes: [
      "Three-way merge engine — when multiple mods provide the same game file, their changes are merged using vanilla as the base. Non-overlapping changes from both mods are kept, conflicts resolved by load order",
      "Contextual pattern scan — JSON patches can now be applied on top of structural mods (save editor mods that change file size). Uses surrounding bytes as a fingerprint to locate the exact item record, even when offsets shift",
      "Standalone overlay + JSON patch coexistence — mods like AQS and Infinity Durability can now work together on the same file without corrupting data",
      "Mod bundle folders — subfolders containing multiple JSON mod variants (e.g. x2, x5, x10, x20) now display as a collapsible card with radio selection. Only one variant active at a time",
      "Manifest-free file replacement mods — mods with a files/ directory containing game files are now detected even without manifest.json",
      "Standalone overlay conflict detection — conflict checker now parses standalone PAMT files to detect when two mods replace the same game file",
      "Same-author conflict suppression — mods from the same author no longer trigger conflict warnings",
      "Removed auto-update and Nexus Mods integration — stripped all external API calls and dependencies",
      "Fixed CSP blocking outbound API calls in v1.0.6b",
    ],
  },
  {
    version: "1.0.7",
  ...
  },
  {
    ...
    changes: [
      ...
      "Added version display in the titlebar",
  ...
  },
  {
    ...
    changes: [
      ...
      "Multi-format import — JSON, ZIP, and folder detection",
